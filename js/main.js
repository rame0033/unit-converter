...
convertBtn.addEventListener("click", function () {
    ...
    try {
        ...
        lengthEl.textContent = `${baseNum} meters = ${(baseNum * meter2feet).toFixed(3)} feet | ${baseNum} feet = ${(baseNum * feet2meter).toFixed(3)} meters`;

        // volume conversion (liter to gallon and vice versa)
        volumeEl.textContent = `${baseNum} liters = ${(baseNum * liter2gallon).toFixed(3)} gallons | ${baseNum} gallons = ${(baseNum * gallon2liter).toFixed(3)} liters`;

        // mass conversion (kg to pound and vice versa)
        massEl.textContent = `${baseNum} kilograms = ${(baseNum * kg2pound).toFixed(3)} pounds | ${baseNum} pounds = ${(baseNum * pound2kg).toFixed(3)} kilograms`;
